...
# ---------------- 例5-12：K/(s+1)^3 ----------------
# 相频 φ = −3 arctan(ω)，令 φ = −180° ⟹ ω = √3 = 1.732（原脚本把 √10 = 3.162 误植进来，
# √10 是含 0.1s+1 环节的另一道题的 ωx，与本例无关；现统一由 ct.margin 取 wg，不再写死）
...
for K, color, ls in ((4, fs.MAG, "-"), (10, fs.PHA, (0, (6, 4)))):
    ...
    if K == 4:
        ...
        wc4, wg4, pm4 = float(wc), float(wg), float(pm)
        pha_wc4 = float(np.interp(np.log(wc), np.log(w), pha))
        axm.axvline(wc, color=fs.SUB, linewidth=0.9, linestyle=(0, (4, 3)))
        axp.axvline(wc, color=fs.SUB, linewidth=0.9, linestyle=(0, (4, 3)))
axm.axvline(wg4, color=fs.SUB, linewidth=0.9, linestyle=(0, (2, 3)))
axp.axvline(wg4, color=fs.SUB, linewidth=0.9, linestyle=(0, (2, 3)))
# 两条竖线只隔 0.15 dec，标签必须分开放：都沉到幅频图下部的空带（曲线在该段已降到 −20 dB 以下），
# 再用细引线连回各自的交点；引线全程走在曲线下方，不会横穿。
axm.annotate("ωc = %.3f" % wc4, xy=(wc4, 0.0), xytext=(0.70, -66),
             color=fs.INK, fontsize=11, ha="center", va="center",
             arrowprops=dict(arrowstyle="-", linewidth=0.8, color=fs.SUB))
axm.annotate("ωg = %.3f" % wg4, xy=(wg4, -6.02), xytext=(2.90, -66),
             color=fs.SUB, fontsize=10.5, ha="center", va="center",
             arrowprops=dict(arrowstyle="-", linewidth=0.8, color=fs.SUB))
...
# γ 落在相频曲线的右上空区（曲线在 ω>1 已降到 −155° 以下，右上整片够不到），引线连回曲线上的点
axp.annotate("γ = %.1f°" % pm4, xy=(wc4, pha_wc4), xytext=(9.0, -60),
             color=fs.PHA, fontsize=11, ha="left", va="center",
             arrowprops=dict(arrowstyle="-", linewidth=0.8, color=fs.SUB))
