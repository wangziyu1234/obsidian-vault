...
| 坐标变换 | `sysT = ss2ss(sys,T)`，约定 $x_{\mathrm{新}}=T x_{\mathrm{旧}}$ |
...
> - 幅值裕度换算为 dB：$h(\mathrm{dB})=20\lg Gm$（`Gm` 是倍率，对应 $\gamma$ 由 `Pm` 直接给）；若存在多个交越点，结合完整奈氏曲线或 `allmargin` 判断，勿仅凭一个正裕度判稳。
...
> [!tip] 每次变换都回验
> 坐标变换检查 $A_{\mathrm{新}}=TAT^{-1}$、$B_{\mathrm{新}}=TB$、$C_{\mathrm{新}}=CT^{-1}$；传函用 `tf(sys)` 对照，反馈用 `eig(A-B*K)` 对照，李雅普诺夫方程检查残差 `A'*P+P*A+Q`。
