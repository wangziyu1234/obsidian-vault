...
BLOCKS = [
    ('02-2 元部件、电路与机械建模', [
        '- **机械-电路相似系统**：力 $\\leftrightarrow$ 电压、位移 $\\leftrightarrow$ 电荷；$m\\leftrightarrow L$、$f\\leftrightarrow R$、$K\\leftrightarrow\\frac1C$。弹簧-质量-阻尼器 $m\\ddot x+f\\dot x+Kx=F$ 给出 $\\frac XF=\\frac1{ms^{2}+fs+K}$。',
        '- **齿轮系折算口诀**：**惯量、黏性摩擦按速比平方折算，转矩按一次方折算**——$J_{eq}=J_1+\\frac{J_2}{i^{2}}$、$f_{eq}=f_1+\\frac{f_2}{i^{2}}$、$M\'_c=\\frac{M_c}i$（$i=\\frac{\\omega_1}{\\omega_2}>0$）。',
        '- **电路建模两法**：① 列微分方程 $\\to$ 拉氏变换（通用）；② **复阻抗法**——串联相加、并联 $\\frac{Z_1Z_2}{Z_1+Z_2}$，输出开路时按**分压** $G=\\frac{Z_2}{Z_1+Z_2}$。常用：$R|C$ 得 $\\frac1{RCs+1}$、$C|R$ 得 $\\frac{RCs}{RCs+1}$。',
        '- **运放电路**：只在**负反馈、线性未饱和**区才能用**虚短虚断**（不能用于正反馈比较器或饱和）；反相接法 $G=-\\frac{Z_f}{Z_0}$。**两个易错**：① 每一级反相都带负号（偶数级抵消、奇数级留一个）；② 求整个模拟电路传函要**逐级写出 $-Z_f/Z_0$ 再按结构图化简**，跨级反馈引线按比较点处理。',
    ...
    ('02-6 典型系统模型·常用元部件与过程控制', [
        ...
        '- **电枢控制直流电动机**：三方程 $=$ 电压平衡（反电势 $E_a=C_e\\omega_m$）、电磁转矩 $M_m=C_mi_a$、转矩平衡。忽略 $L_a$ 得 $T_m\\dot\\omega_m+\\omega_m=K_mu_a-K_cM_c$，传函 $\\frac{K_m}{T_ms+1}$ 与 $-\\frac{K_c}{T_ms+1}$（**扰动为负**）。',
        '- **单容水槽**：$Q_o=\\kappa\\sqrt h$ 取 $h_0>0$ 线性化，$K_q=\\frac{\\kappa}{2\\sqrt{h_0}}$、**液阻 $R=\\frac1{K_q}$**；$\\frac{\\Delta H}{\\Delta Q_i}=\\frac{1/K_q}{(A/K_q)s+1}$；**纯运输延迟在传函上乘 $e^{-\\tau s}$**。',
        '- **电加热炉**：$C\\frac{d\\theta}{dt}=P-K(\\theta-\\theta_0)$ 改写成**偏差**形式 $C\\dot x+Kx=p$，得 $\\frac{X}{P_\\Delta}=\\frac{1/K}{(C/K)s+1}$——**与单容水槽同构**；操纵量为电压时还要线性化功率。',
