...
- Git：`main` 分支，远程 `git@github.com:wangziyu1234/obsidian-vault.git`；每次修改完必须主动 `git add -A` → commit → push
- 提交信息：中文、按用途加前缀——`fix:` 修错误 / `refactor:` 结构调整 / `docs:` 笔记内容 / `chore:` 杂项清理
- 笔记主体：**控制理论**（`自动控制原理\` 46 篇 + `现代控制理论\` 19 篇）、**数学**（高数目录 + 线代目录，共 96 篇）、**英语**（16 篇）；附件 `附件\` 全部 png；另有 `README.md` 库总览
