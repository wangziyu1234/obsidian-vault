...
- **真凶画像**：模式是 `<原名>_<13位epoch毫秒>`，而且会**反复叠加**（`.trash` 里留着 `伯德-例512_1788888780286_1789018182803.jpg` 这种二次后缀，还有同名 `.jpg` 孪生），说明对方客户端有个「附件重命名/文件名格式化」规则（`${attachmentName}_${timestamp}` 这类写法）在**重复处理同一批附件**——正好就是那 6 篇笔记引用的 9~11 张图（04-1、04-2-2、04-4、05-2-b、05-3-b-2、06-1-b）。`附件\.nomedia` 是 Android 标记，说明这台手机确实同步过这个库。**修的地方在那台设备上**：查手机 Obsidian 的插件/设置里有没有附件自动重命名（或文件名模板），以及手机上有没有别的同步/文件管理 App 指向本库或 `OneDrive\Apps\remotely-save\`。
- **下次怎么一眼确认是"外面干的"**：`check-notes.ps1` 现在会顺带查**云端镜像** `%USERPROFILE%\OneDrive\Apps\remotely-save\obsidian\附件\`（OneDrive 客户端维护的云上副本、文件名明文）。若带后缀的名字**只出现在镜像里、本库还是原名**，就证明云上被改名了、本机无辜——直接去手机端关那个自动重命名。
