...
| **③ 二项展开** | $A$ 可拆 $A=B+C$ 且 $BC=CB$（常取 $A=kE+B$） | $A^n=(kE+B)^n=\sum\limits_{k=0}^{n}\binom{n}{k}k^{n-m}B^{m}$；若 $B$ 为严格上三角幂零（$B^s=O$）则取前 $s$ 项 |
...
> $A=(E+B)^n=E+nB+\dfrac{n(n-1)}{2}B^2$（$B^3$ 及以后全 0）。单位矩阵与任何同阶方阵可交换 ⟹ 二项式可正常展开。
...
| $(A^{-1})^{*}=(A^{*})^{-1}=\dfrac{A}{\lvert A\rvert}$（$A$ 可逆）| $(A^{k})^{*}=\lvert A\rvert^{k-1}A^{*}$ |
| $(kA)^{*}=k^{n-1}A^{*}$                             | $(A^{T})^{*}=(A^{*})^{T}$                  |
...
> [!example] ✏️ **例5：秩不等式**
> 设 $A_{3\times4}$，$B_{4\times3}$，若 $AB$ 可逆，求 $r(A)$：$r(A)\le3$ 且 $r(A)\ge r(AB)=3$（因 $r(AB)\le r(A)$），故 $r(A)=3$（列满秩）。同理 $r(B)=3$（行满秩）。
