...
- **初等矩阵的行列式**：$\lvert E_{ij}\rvert=-1$、$\lvert E_{i}(k)\rvert=k$、$\lvert E_{ij}(k)\rvert=1$——与行列式三性质**一一对应**（换行变号 / 某行乘 $k$ 行列式乘 $k$ / 倍加不变），即由 $\lvert PA\rvert=\lvert P\rvert\lvert A\rvert$ 即可推回这些性质。
...
3. **求 $P,Q$ 使 $PAQ=\begin{pmatrix}E_r&O\\O&O\end{pmatrix}$**：拼 $\begin{pmatrix}A&E_m\\E_n&O\end{pmatrix}$，对上半块 $[A\mid E_m]$ 做**行变换**、对左半块 $\begin{bmatrix}A\\E_n\end{bmatrix}$ 做**列变换**，把 $A$ 位置化为 $\begin{pmatrix}E_r&O\\O&O\end{pmatrix}$——此时右上块 $E_m\to P$、左下块 $E_n\to Q$，即 $PAQ=$ 标准形。
